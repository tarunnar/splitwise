...
class GroupUserBalancesView(APIView):
    def get(self, request, *args, **kwargs):
        ...
        if not user_id:
            raise exceptions.ValidationError(
                {
                    constants.MESSAGE_KEY_STR: constants.USER_ID_MISSING
                }
            )
        group_balances_response = GroupBalanceService.get_user_balances(
            group_id,
            user_id
        )
        if group_balances_response[constants.STATUS_KEY_STR] == constants.SUCCESS_STATUS:
            return JsonResponse(
                ...
                data={
                    ...
                    "group": group_balances_response.get("group_balances")
                }
            )
        return JsonResponse(
